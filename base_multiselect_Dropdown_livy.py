    ...
    if n_clicks > 0:
        ...
        while(True):
            ...
            if statement_response["state"]=="available":
                ...
                depth_i=0
                traces=[]
                x=dfe.Year[(dfe['property_code'].isin(property)) & (pd.DatetimeIndex(dfe['forecast_period_end_date']).year > yr)]
                for i in property:
                    depths[depth_i]=dfe.life[(dfe['property_code']==i) & (dfe['forecast_period_end_date']).year > yr)]
                    mydict={'x': x, 'y':depths[ic], 'type': 'lines', 'name': property[ic]}
