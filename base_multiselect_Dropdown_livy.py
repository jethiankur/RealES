...
def update_graph(input_value,property,year):
    ...
    if n_clicks > 0:
        ...
        properties = "('"+myString+"')"
        templated_string=template.substitute(modifier=properties)
        ...
        while(True):
            ...
            if statement_response["state"]=="available":
                ...
                #use the dataframe to create traces and plotly
                yr=int(year)
                depths = [[] for i in range(len(property))]
                depth_i=0
                traces=[]
                x=dfe.forecast_period_end_date[(dfe['property_code'].isin(property)) & (pd.DatetimeIndex(dfe['forecast_period_end_date']).year > yr)]
                for i in property:
                    depths[depth_i]=dfe.avg_net_income_pq[(dfe['property_code']==i) & (pd.DatetimeIndex(dfe['forecast_period_end_date']).year > yr)]
                    mydict={'x': x, 'y':depths[depth_i], 'type': 'lines', 'name': property[depth_i]}
